...
文件分工：本文件是加载与谓词求值；调用选择、记录本步、当前步显示与对话模式（地址栈、路由、压帧弹帧、帧替换）
在 taskdef_step.py 的混入类 StepMixin 里，TaskDefinition 在本文件末尾继承它。
"""
...
from tod_kernel.tools import EXCEPTION_TOOL, STATIC_TOOLS
...
# ───────────────────────── 任务定义对象 ─────────────────────────
# 当前步与对话模式那一半在 taskdef_step.py（StepMixin）。这里在文件末尾才导入它：它从本模块导入谓词求值助手，
# 那些助手必须先定义好。本模块同时把那边的公开名字原样导出，外部照旧从 taskdef 导入。

from tod_kernel.taskdef_step import (AT_INDEX, AT_LOOP, AT_LOOP_LAST, AT_NOTE, AT_STAGE, CLARIFY_FUNCTION,
                                     EXCEPTION_KIND_KEY, FRAME_INPUT, FRAME_PATTERN, FRAME_REASKED, FRAME_REASK_LIMIT,
                                     FRAME_RESULT, FRAME_STEP, KIND_BROKEN, KIND_GROUP_LIMIT, KIND_PASS_DONE,
                                     LOOP_FROM, LOOP_NTH, LOOP_TO, NO_PRECONDITION, REASON_GOAL_HOLDS,
                                     REASON_LIMIT_NULL, REASON_UNTIL_HOLDS, REF_NULL_NOTE, STACK_INSERTS, STACK_MAIN,
                                     STEP_INDEX, STEP_LOOP, STEP_STAGE, StepMixin, TRAIL_FORWARD, TRAIL_KEY,
                                     TRAIL_SKIPPED_STAGES, TRAIL_SKIPPED_STEPS)  # noqa: E402


class TaskDefinition(StepMixin):
