...
- 验证脚本 `python3 -m tod_kernel.verify` 回放模式全过，退出码 0：九个场景加十组检查，共 972 条断言（第四步收尾时是 612 条）。
- 标注集 39 条，逐项功能准确率 50/51 = 0.980，逐句全对率 38/39 = 0.974，下限 0.8。这是第二轮录制的结果，第一轮只有 0.782，原因与处理见 G5。
- kernel.py 只改了一处，就是 2026-09-18 裁定问题二同意的参数核对，逐行见 G3。
- 没有提交，没有推送。
...
| 文件 | 改了什么 |
|---|---|
| `kernel.py` | 调用选择核对参数名：必填参数都要在，清单外的参数一个都不许有，可选参数可以不写（G3） |
| `tools.py` | 新增对话理解工具 `understand`，以及它用到的配对表、对话功能常量、输出 Schema 现算、结构校验、六条规范化、按行为列表落数据；判读回复连实现、前置条件、静态工具表登记、固定指令一起删掉；询问加了四个可选参数 type、about、adopt_to、options，会登记上一问，会把待处理拼成句子填进模板占位 `{pending}`，说完清空待处理；`Tool` 与 `ToolSpec` 加 `optional_params`，`ToolSpec` 另加 `required_slots`；`build_table` 加 `confidence_floor` 参数 |
| `context.py` | 新增 `candidate_functions(question)` 与 `writable_paths(slots_meta, question, data)`；当前数据段里的结构化值（列表、字典、推迟标记）改写成 JSON，字符串照旧原样 |
| `taskdef.py` | 槽位元数据认可选布尔键「使用者可写」；步骤参数核对放行可选参数；按静态工具表的 `required_slots` 校验必备槽位，缺槽位或类型不对都报加载错误 |
| `task_defs/glossary.json` | 按 4.11 节改：加槽位「上一问」「待处理」；「释义草稿」标使用者不可写；念草稿那一问的参数加 type、about、adopt_to；循环段第 2 步改为对话理解；回复的提问模板句首加 `{pending}` |
| `task_defs/任务定义.schema.json` | 槽位元数据加可选布尔键「使用者可写」 |
| `task_defs/eval/对话理解标注集.json` | 新文件，39 条 |
| `task_defs/recordings/glossary.json` | 整份重录，共 13 条：生成术语释义 4 条，对话理解 9 条 |
| `task_defs/recordings/对话理解标注集.json` | 新文件，39 条 |
| `config.example.json` | 加 `"confidence_floor": 0.6`（可选键，读不到时用默认值 0.6） |
| `verify.py` | 术语澄清原来的三个场景换成六个新场景；新增两组检查：标注集评测、对话理解机器检查；有六个共用断言按新情况调整（G4 末尾列出） |
| `console.py` | 菜单加「对话理解标注集」一项（编号 10），逐条打印；修了自由输入的一个既有错误（G6 第 1 条） |
| `observatory.html` | 模型卡遇到对话理解的调用记录时显示原话与行为列表；上下文变化表加一行「行为列表」；「谁填的」标签认 `writer_roles` 的通配键「*」 |

公开仓边界已检查：tod_kernel 目录下没有设计仓的名称或路径。代码、提示词和界面新写的文字里，也没有出现「游标」「步骤组」「全局步骤号」「宿主」「行动」，没有循环义的「轮」。
...
为什么要改：材料接入登记的询问不带 type 等参数，而且它的参数必须与第二步留下的旧运行文件逐位相等，所以不能给它补空值，只能让参数可以不写（2026-09-18 裁定问题二）。内核通过 `getattr` 读工具对象上的 `optional_params`，代码里不出现任何工具名或槽位名。「内核源码不出现槽位名与工具名」那组检查照旧全过，词表另加了「对话理解」「上一问」「待处理」「修改意见」四个词。
...
- 工具调用序列与依据序号；
- 每次对话理解规范化后的功能列表；
- 本步段里的上一问、候选功能集与可写路径清单；
- 「回复」「上一问」读过后清空；
- 落到了哪个槽位（确认释义、修改意见、术语、推迟标记、待处理）；
- 念草稿那一问的话逐字等于预期句，提问后再确认的场景里，预期句句首是待处理拼出的那句；
- 念草稿时登记的上一问；
- 循环次数与当前步终态、交付物；
- 各场景共用的完整性、记录方、运行文件与重放检查。

机器检查组有 29 条断言，覆盖这些内容：
- 候选功能集：四种上一问类型加上一问为空的情形；
- 可写路径清单：用含列表与项字段的假槽位表算清单，另有建议类上一问开放修改意见、当前值为空两种情形；
- 输出 Schema：现算出来的枚举对不对；用 jsonschema 校验它本身合法，清单内的写值通过，写不可写的槽位被拦下；
- 六条规范化各有例子，第 1 条有三个，第 3 条另有一个例外；
- 四条不经场景的落数据路径：求证类同意、代码澄清的三个选项、模型写的自由文字选项、把握低于阈值；
- 待处理拼句两例；
- 缺三个必备槽位时各报加载错误；
- 模型不可达时对话理解记已失败（由退役的「模型不可达」场景移过来，2026-09-18 裁定问题九）。

为第五步调整过的六个共用断言：
1. 询问的写入目标断言只看写入目标那一条变更，因为询问现在可能还登记上一问、清空待处理。
2. 非询问工具的「只写可写槽位」断言：对话理解的可写槽位由数据决定，改为核对它只写了使用者可写的槽位与三个必备槽位。
3. 当前步检查里的步骤说明从「判读回复：确认还是修改」改为「理解使用者的回复」。
4. 上下文包检查里系统提示的固定指令，改核对第五步 4.4 节的原文。
5. 观测台检查的运行参数换成新的回答。
6. 控制台菜单改为 11 项。
...
标注集的构成：
- 建议类上一问 24 条：第五步 4.9 节七种情形各 3 条，另有无关句 3 条；
- 求证、选择、请求三类上一问各 5 条。

第 1、4、7、10、13、16、19 条分别对应 4.9 节表里的七行。比对规则写在标注集文件的「说明」里：
- 逐项只比功能与内容；把握、规范化修订、「回应上一问」只记录，不参与比对；
- 值写「*」表示任意非空；写 `{"含": [...]}` 表示字符串里含这些词；
- DEFER 的内容为空时，按推迟目标槽位的取法算出槽位再比。

两个比率的算法：
- 逐项功能准确率：功能对上的项数，除以每条里期望项数与实际项数较大者之和；实际多出来的项也算错。
- 逐句全对率：功能与内容全部对上、而且项数相同的条数，除以总条数。

两轮录制，数字都照实列出：

| 轮次 | 真模型调用条数 | 逐项功能准确率 | 逐句全对率 | 主要错法 |
|---|---|---|---|---|
| 第一轮 | 场景 13 条，标注集 39 条 | 43/55 = 0.782（低于下限） | 28/39 = 0.718 | 同意时另加一项把草稿原样写进确认释义（第 1、2、3、30 条）；整段改写判成 DENY（第 7、9 条）；「嗯。」与「你看着办吧」判成同意（第 19、21 条）；「这版不行，重新写一份」判成 DENY 加修改意见（第 12 条） |
| 第二轮 | 场景 9 条（另 8 条命中已录的草稿请求），标注集 39 条 | 50/51 = 0.980 | 38/39 = 0.974 | 只剩第 33 条：「先放一放。」在选项序号 3 之外还多出一项 DEFER |

两轮之间只改了一处：输出 Schema 里每个功能那一支的说明文字（`tools.py` 的 `FUNCTION_NOTES`，写进 Schema 的 description）。第 4.4 节的固定指令、本步段、系统提示一个字没动。这些说明是我加的，文档里没有，写法取自 4.1 节的名词、4.5 节的内容形状表和 4.7 节「建议类上一问只允许两种结果」的裁定，例如「同意时内容为空，不要把建议原样再写一遍」「只有语气词、或说法模棱两可时才产出 CLARIFY，不要猜成同意」。风险要说明：第二轮是看着第一轮的错法改的说明，准确率里有贴合这 39 条的成分，演示时换新的说法可能更低。说明文字该不该留、该怎么写，列为待裁定问题（G7 第 1 条）。

加说明文字之前还改过一次 Schema 的形状，发生在正式录制前的冒烟运行里。起初每项的「内容」是一个不分功能的多选一（anyOf），模型会给 DEFER 配上选项序号、给 REQUEST 配上槽位。改成按功能分支以后，功能一定下来，内容形状就被 Schema 约束住。三栏枚举仍然写死在 Schema 里：功能、写值的槽位、与槽位配对的路径。

录制文件里的条数：`glossary.json` 13 条，`对话理解标注集.json` 39 条。录完后用回放跑了一遍全量验证，统计实际查到的请求，把没被查到的旧条目清掉；两份文件的每一条都被用到。补录时用的是临时脚本：录制文件里有这条请求就回放，没有才调真模型。这样六个场景里共用的「需求确认」首稿请求只录一次，不会被后录的回答覆盖。这个脚本不入库。

## G6 实现时自己定的取舍（主会话说这类小事由我定、写进报告）

1. **控制台自由输入的既有错误**：`run_free_input` 把模型调用件存进变量 `call`，传参却写成 `call_model=call_model`。这应该是上一步改名留下的，自由输入一进去就会报变量未定义。已改正，并连真模型跑通一次：术语澄清，答「可以，就这样。」，37 条通用检查全过。负责人演示走的就是这条路。
2. **本步段写法**：沿用上下文包的 `ContextPack.step`，所以开头是「本步执行工具「对话理解」，按系统提示工具目录中它的固定指令：上一问：类型 建议，槽位 释义草稿，路径 []，选项 （空），采纳到 确认释义；候选功能集：…；可写路径清单：术语 []；原文片段 []；修改意见 []；确认释义 []。」路径写成 JSON 数组。
3. **当前数据段装什么**：全部使用者可写的槽位，最后一行是「回复（待理解的原话）：…」。第 4.3 节把回复原话列为输入，但没说放在哪一段，我放在当前数据段。释义草稿不在这一段（它使用者不可写），模型经对话历史里那一问的原话看到草稿。
4. **Schema 里提问内容的说明**：给「问」加了说明「槽位名，或使用者问到的那个词的原话」。加之前，模型把「「产出」是什么意思？」整句填进「问」，拼出来的句子是「你问『「产出」是什么意思？』是什么意思」。
5. **代码澄清的选项**：按裁定问题四，第 1 条规范化产出的澄清恒为固定三项，不再「按上一问类型定」，并带上「原问」。上一问本身就是澄清之后再问的选择类时，原问取它记着的那一问，不一层套一层。
6. **「原问」怎么存**：待处理的澄清项带「原问」；由它再问时，询问把原问也记进上一问（上一问因此多一个键「原问」）。对话理解看上一问有没有这个键，来分辨选项是代码写的固定三项，还是模型写的自由文字。
7. **末尾新增的几项写值**：同一句里有好几项写到 `["+", 字段]`，它们并进同一个新项，不各自新建一项。
8. **每个槽位只出一条变更**：落数据先在工作副本上改，最后每个槽位出一条变更，旧值取执行前的值，同一个更新组。
9. **把握阈值怎么传进工具**：工具不读配置，由程序入口经 `build_table(confidence_floor=…)` 传入。控制台读配置键 `confidence_floor`；验证脚本用默认值 0.6。
10. **写值类型核对的范围**：对象槽位按键写的值没有元数据可以核，一律放行；列表项整项写入时，只核它的键在「项」字段表里。
11. **对话理解调用记录多出的键**：`reply`（原话）、`acts`（规范化后的行为列表，每项带「落成」，被改动过的项带「规范化」与「模型原值」）、`dropped`（规范化删掉的项）、`problem`（第 1 条规范化的原因，没有就是空）、`landed`。`parsed` 仍是模型原文解析出的对象。
12. **「谁填的」标签**：对话理解写进哪个槽位由数据决定，`writer_roles` 用通配键「*」标「使用者」，待处理标「系统」。

## G7 待裁定的问题

1. **Schema 里的功能说明文字（G5）要不要留，写法由谁定**。它不是 4.4 节的固定指令，但它进了请求，也就进了录制哈希。去掉它，标注集准确率会回到 0.8 以下。建议：留下来，把这段文字写进 05 文档 4.5 节，作为 Schema 的一部分定稿；以后再改就按改提示词处理，要重录。
   裁定（2026-09-18）：说明文字留下，按 `FUNCTION_NOTES` 原文逐字写进 05 文档 4.5 节，作为 Schema 定稿的一部分，不再改一个字；贴合标注集的风险一并记进文档。
2. **模型写的澄清在术语澄清里几乎落不了地**。「嗯。」这类句子，模型产出的 CLARIFY 带自由文字选项，例如「确认释义／提出修改意见／重新生成释义」。再问时上一问的槽位是不可写的释义草稿，按裁定问题四，使用者选任何一项都记「选择项不可写，未落」，下一次循环照常再念草稿。负责人演示时如果故意说含糊话再选「1」，看到的会是：选了没有生效，草稿又念一遍。流程不会卡死，但读起来像没听见。请确认这是本步接受的表现，还是要让模型写的澄清也按原问落。
   裁定（2026-09-18）：原问是建议类时弃用模型写的选项，恒用代码固定三项再问，序号 1、2、3 按原问落；原问是其他类型时仍用模型写的选项（改动见 G8）。
3. **第 33 条「先放一放。」多出一项 DEFER**。模型同时给了 AFFIRM（选项序号 3）和 DEFER（确认释义）。落数据时两项都会走推迟：确认释义标两次推迟标记，待处理多出一条重复的推迟。第五条规范化只限回应类至多一项，管不到这种重复。要不要加一条规范化，这里只报告，不自行加。
   裁定（2026-09-18）：加第 7 条规范化——两项落到同一目标同一动作时留前一项，记「重复项」；机器检查组加一例（见 G8）。
4. **推迟收尾的结局**：推迟并顺手改术语场景按裁定问题六收尾——任务完成，交付物是 `{"已推迟": true}`，待处理里那句推迟不会被说出来。场景断言就是按这个写的，这里再提一次，方便负责人演示时对照。
   裁定（2026-09-18）：确认按完成结局收尾，交付物是推迟标记。

## G8 四个问题裁定之后的改动（2026-09-18）

主会话对 G7 四个问题的裁定已写进 05 文档：功能说明文字逐字留下，不再改；推迟后任务以完成收尾。另外两条要改代码，本节记这两处。两处都只改规范化与落数据的代码，不改请求内容，所以录制文件不用重录。

1. **模型写的澄清改用固定三项**（`tools.py` 的 `land_acts`）。条件是：原问是建议类，这条 CLARIFY 又是模型写的——不带「原问」，也不是第 6 条规范化产出的。满足时，进待处理的澄清项弃用模型写的选项，换成代码固定三项，并带上原问。于是「嗯。」之后再问，选项固定为 1 确认这一稿、2 修改这一稿、3 先放一放，使用者选哪一项都按原问落。原问是其他类型时，仍然用模型写的选项。这一项的「落成」写成「待处理加一条澄清（原问是建议类，选项换成代码固定三项）」。
2. **第 7 条规范化**（`normalize_acts` 末尾，新函数 `_landing_key`）。两项落到同一目标、同一个动作时，留前一项，被删的一项记进留下那一项的「重复项」，也进调用记录的 `dropped`（规则名「第 7 条」）。本步只认「推迟」这一种动作：DEFER 一项；以及选择类上一问由代码固定三项再问、使用者选了序号 3 的 AFFIRM。同槽位同路径的写值原本就由第 2 条管。

机器检查组加了两条断言，从 29 条变成 31 条：一条是第 7 条规范化的例子，一条是原问为建议类时模型澄清换成固定三项。复跑 `python3 -m tod_kernel.verify`，回放模式全过，退出码 0，共 974 条断言。

标注集结果随之变化：第 33 条「先放一放。」原来多出的一项 DEFER，被第 7 条删掉，于是逐项功能准确率 50/50 = 1.000，逐句全对率 39/39 = 1.000。这个满分有两个来源：一是这 39 条上看着错法改过的说明文字，二是第 7 条规范化，所以不代表演示时换新说法也能全对。

公开仓里 `backend/app/domain/model.py` 与 `backend/tests/test_domain_model.py` 的未提交改动不是本步的，没有碰过。

## G9 增补：对话模式层与观测台插入段（2026-09-18）

### G9.1 结论

- 第六项「对话模式层」、第七项「观测台插入段」都已做完。`python3 -m tod_kernel.verify` 回放模式全过，退出码 0：十个场景加十组检查，共 1124 条断言（增补前 974 条）。
- kernel.py 本批没有改动，全仓仍只有问题二那一处参数核对的改动（G3）。
- 控制台自由输入连真模型跑通一次。初始输入给「术语=需求确认」，使用者问「项目干系人是什么意思？」，系统先说了两句白话：「项目干系人指与项目有利益关系、能影响项目或被项目影响的个人或群体，如客户、用户、管理层等。在此处它等同于“利益相关方”，即参与需求确认过程的关键人员。」然后再念草稿；使用者确认后任务完成，49 条通用检查全过。
- 没有提交，没有推送。公开仓 backend 下两个文件的未提交改动不是本步的，没有碰过。

### G9.2 改了哪些文件

| 文件 | 改了什么 |
|---|---|
| `task_defs/patterns.json` | 新文件。路由表四行：REQUEST→答疑，CLARIFY→澄清，DEFER→推迟，INFORM（把握低于阈值）→求证。四个模式各两步，推迟模式按裁定问题一先告知、再标记 |
| `taskdef.py` | 当前步改为地址栈 `{"主线", "插入"}`，原来的单个地址仍当输入认。`select_call` 有帧时从栈顶帧往下找能做的步骤；`record_step` 负责四件事：按对话理解返回值里的 `routes` 压帧（话里第一项在栈顶）、帧内步数加一并记「结果」、做完弹帧、本帧再问与帧替换。`step_text` 不在插入段时与原来逐字相同，在插入段时后接一句插入段做到哪；`step_view` 另加「插入」列表。另加 `load_patterns`：用与任务定义同一套步骤校验，另查三种模式引用与路由表。只有用了对话理解的任务才并进模式，定义里多一个「对话模式」块，材料接入登记的任务开始事件不变 |
| `tools.py` | 新增三个工具：告知（种类 notice 的消息，不等回答）、标记推迟、答疑（模型端口）。对话理解改为列出待路由的行为，插入段里不嵌套：要路由时换成本帧再问，澄清帧里选 3 换成帧替换。询问加两个可选参数 `text` 与 `original`。退役：`PENDING_SLOT`、待处理拼句、`{pending}` 占位、落数据里的「进待处理」各分支。`build_table` 自动登记模式里用到的工具 |
| `context.py` | 阶段名改从地址栈的主线读；对话历史把告知装成「系统：」一行，来源说明里单独写「告知 n 句」 |
| `console.py` | 宿主循环认告知消息：打印「系统：」行，不要回答；定义文件列表排除 patterns.json；菜单变成 10 个场景 |
| `task_defs/glossary.json` | 去掉「待处理」槽位与 `{pending}` 占位 |
| `verify.py` | 新增「含糊后选择」场景，改写推迟、提问两个场景，其余场景的当前步比对改按地址栈；机器检查组加路由、压帧、选帧、弹帧、本帧再问上限、帧替换、插入段不嵌套、退役核对；两条通用检查认可告知消息；JSON Schema 检查排除 patterns.json |
| `observatory.html` | 第七项四处，见 G9.6 |
| 录制文件 | 两份整份重录：系统提示的工具目录多了三个工具，旧请求哈希全部失效 |
...
三个与插入段有关的场景各自核对了下面这些事。

推迟并顺手改术语：
- 待路由的行为只有 DEFER 一项，目标是确认释义；术语那一项直接写入；
- 告知的话逐字是「『确认释义』先放着，回头再问。」，不等回答；
- 标记推迟之后任务以完成收尾；
- 当前步序列依次是：压推迟帧 → 告知做完第 1 步、结果记下那句话 → 标记做完第 2 步、弹帧回到主线原地址。

提问后再确认：
- 待路由的行为是 REQUEST，问的是「产出」；
- 答疑那次的本步段、当前数据段形状对，解释放在返回值的「输出」；
- 告知的话逐字等于这段解释；
- 第二次念草稿的话逐字等于原模板，第二次对话理解的对话历史里有那句告知；
- 当前步序列依次是：压答疑帧 → 记结果 → 弹帧 → 主线回段首，第几次加一。

含糊后选择：
- 「嗯。」理解成 CLARIFY，输入是原问；
- 再问的话逐字是「你是想：1 确认这一稿 2 修改这一稿 3 先放一放？」，登记的上一问带原问；
- 使用者答「1」，理解成 AFFIRM 选项序号 1，按原问把草稿采纳进确认释义；
- 当前步序列依次是：压澄清帧 → 弹帧。

### G9.4 录制与标注集

- 真模型调用共 55 条：场景 16 条，标注集 39 条。`recordings/glossary.json` 共 16 条，其中文本形状 5 条（4 条写草稿、1 条答疑），JSON 形状 11 条（对话理解）；`recordings/对话理解标注集.json` 共 39 条。回放统计确认每一条都被用到。
- 标注集结果：逐项功能准确率 49/50 = 0.980，逐句全对率 38/39 = 0.974。唯一不一致是第 33 条：上一问是选择类，使用者说「先放一放。」，模型只给了 DEFER，没有给 AFFIRM 选项序号 3。这种情形在实际运行里的后果见 G9.7 第 1 条。
- 输出 Schema 的功能说明文字按上一批裁定一字未动；请求变了，只是因为系统提示里的工具目录变了。

### G9.5 答疑的固定指令原文（进系统提示的工具目录，请逐字写进 05 文档 4.12 节）

> 你是答疑员。使用者在任务进行中问了一个词或一句话是什么意思，问的内容在【本步】段里。结合当前数据里的槽位现值与对话历史，用一到两句白话说清它在这个任务里指什么：槽位现值或对话里有相关内容时照它解释，没有时按需求工程语境的通用含义解释。不反问，不评价也不改写草稿，不写入任何槽位。

答疑每次调用的另外两段固定写法：本步段是「使用者问：{问的内容}」（接在 `ContextPack.step` 那句「按系统提示工具目录中它的固定指令：」之后），输出形状段是「只输出一到两句白话解释正文。」。段的组成是任务进度、对话历史、当前数据（全部槽位）、本步、输出形状。

### G9.6 第七项：观测台插入段（样式依据原型_观测台复杂任务_v3）

1. **时间轴与路径图**：阶段横带之下，给这次运行里走到过的每个模式加一条紫色虚框横带，标签是「插入段 · 答疑 · 2 步」；插入段里的点是紫色；凡是有一端在插入段里的连线都画紫色点线，与重做退回的红色虚线区分；图例加了这两项。
2. **协同记录卡**：插入段里的迭代卡加紫色左边条，卡头加紫色标签「插入段 · 模式「答疑」第 1 步」，位置写「阶段『确认』· 主线位置不动」，卡身仍是现行的五行。告知画成「系统 → 使用者」气泡，底下写「发件箱 · 告知 #n · 不等回答」，没有使用者回答那一栏。对话理解卡在「落了什么」之下加一行「去向」，例如「REQUEST → 路由到模式『答疑』」；插入段里的本帧再问与帧替换也各有一句。模式名取自运行文件里并进来的路由表，页面不写死。
3. **右栏当前步**：拆成「主线」「插入」两行。在插入段里时，插入一行写模式名与做到第几步；不在插入段内时照写，并注明上一次插入段在第几次迭代结束。下方那句说明另加两种情形：压入插入段、插入段跑完回到主线原处。
4. **运行有向图**：每个走到过的模式，在主轨下方画一个紫色虚线框的子图，模式名写在框底。步骤框带迭代号圆点；一条紫色进边从触发它的那一步下来，标「插入 · n 次」；一条紫色回边回到主线上那一步，标「跑完回到主线原处 · n 次」。原型 v3 的结构图里没有模式子图的画法，这个画法是我按「运行有向图」原有的 dg 样式定的，请走查时确认。

截图 5 张放在步骤目录，都取自「提问后再确认」那次运行：`截图_第五步观测台_插入段_路径图_1280.png`、`截图_第五步观测台_插入段_答疑卡与右栏_1280.png`、`截图_第五步观测台_插入段_告知卡_1280.png`、`截图_第五步观测台_插入段_对话理解卡路由去向_1280.png`、`截图_第五步观测台_插入段_有向图_1280.png`。第四步留下的旧运行文件（当前步是单个地址）在新页面上照常显示，插入一行写「不在插入段内」。

### G9.7 待裁定的问题

1. **澄清帧里使用者用话说「先放一放」，而不是报序号**。标注集第 33 条显示，模型对这句话只给 DEFER。在插入段里，DEFER 属于要路由的行为，按「插入段内不嵌套」会被换成本帧再问，而不是裁定问题六的帧替换。结果是再问两次后弹帧，推迟没有落下。建议：澄清帧里的 DEFER，如果目标就是原问的推迟目标，按选项 3 处理，走帧替换。请裁定是否这样改。
2. **帧多了两个键**：「结果」按裁定问题二加；「再问」是为了实现「同一帧最多再问 2 次」而加的计数，裁定里没有写到这个键。另外，询问、告知这类工具的返回值不是字典时，也会记进帧的「结果」。例如澄清帧里询问的返回值「1」会记进去，只作显示，不参与任何判断。
3. **模式子图的画法**（G9.6 第 4 条）是我定的，原型里没有，请确认或给出改法。
4. **完成条件不看插入段**：已按裁定问题一记为遗留。推迟模式因为先告知、再标记，这次不受影响；但任何「插入段里写槽位、写完主线目标就成立」的模式，插入段余下的步骤都不会再跑。

### G9.8 增补四问裁定之后的改动（2026-09-18）

主会话对 G9.7 四个问题的裁定：第 2、3、4 条接受或已记为遗留；第 1 条要改代码，本节只记这一处。

- **改动**：`tools.py` 的 `normalize_acts` 开头加了一步，先于第 2 到第 7 条执行。条件有三个：上一问是代码固定三项的选择类再问（带「原问」）；模型给的 DEFER 目标就是原问的推迟目标；同一句里没有别的回应类项。三个条件都满足时，这项 DEFER 改写成 AFFIRM 选项序号 3，并记下模型原值。于是在插入段里，这句话走帧替换（换成推迟帧），不再是本帧再问。同一句里已经有 AFFIRM 选项序号 3 时不改写，多出的那项 DEFER 仍由第 7 条记成重复项。
- **没改的**：请求内容没有变，所以不用重录；kernel.py 仍只有第五步首批那一处改动。
- **新增检查**：机器检查组加两条，从 38 条变成 40 条。一条核对上面的规范化；另一条核对插入段里「先放一放。」走帧替换、不走本帧再问。
- **结果**：`python3 -m tod_kernel.verify` 回放全过，退出码 0，共 1126 条断言。标注集第 33 条随之一致：逐项功能准确率 50/50 = 1.000，逐句全对率 39/39 = 1.000。

### G9.9 插入段返回后不复述草稿（2026-09-18，05 文档 4.12 节实施裁定第 11 条）

用户验收时发现：答疑跑完、回到主线「念草稿」那一步，系统把整段草稿原样又念了一遍。本节记这处修改。

- **改动**：询问工具在要登记上一问（参数带 type）且没给 text 时，先在事件流里找最近一次类型、槽位、路径都相同的那次登记。找到了，并且要念的值（参数 about 所指位置的当前值）与那次念的相同，话语就换成短句「回到刚才那一稿：请确认，或提出修改意见。」；否则照常按提问模板全文念。短句可以在写入目标槽位（或列表项字段）的元数据里用可选键「再问短句」改写，没写就用这句固定文字。加载器与 JSON Schema 都认这个键，写了就必须是非空字符串。
- **副本放在哪里（按派发说明由我定）**：上一问的登记里多记一项「念的值」，就是这一问念的那个值；用来比对的那次登记从事件流里取：「上一问」这个槽位写入过的非空新值里，最近一条类型、槽位、路径都相同的。对话理解读完后清空上一问不影响比对，因为事件流里的登记一直留着。为此建工具表时给询问工具也绑上了读事件函数。没有选另外两个位置，理由如下：
  - 插入段的「结果」不合用：念草稿这一问在主线上，不在插入段里；而且回到主线时，答疑那一段已经结束并被移除了。
  - 新加一个槽位不合用：槽位清单会进系统提示里的任务定义摘要，系统提示一变，全部录制都要作废重录。
- **没有用短句的情形**：
  1. 参数给了 text 时照说 text，不换短句。插入段「澄清」「求证」两个模式里的问句都是这样给的，所以本帧再问时仍然逐字念固定三项。
  2. 宿主没给读事件函数时，找不到上一次登记，照常全文念。
  3. 原先写的第 3 条已按主会话裁定改掉：澄清段跑满再问上限、回到主线时，中间虽然隔着澄清段里那次选择类登记，比对的仍是最近一次同类型、同槽位、同路径的登记，草稿没变就说短句。
- **其他细节**：
  - 「原问」不带「念的值」，免得插入段的输入与观测台里多出整段草稿。
  - glossary.json 里「上一问」的说明没有补写「念的值」：这句说明会进系统提示，改了就要全部重录。主会话裁定不补、不重录。
- **录制**：请求里只有「提问后再确认」场景第二次对话理解那一条变了，原因是对话历史里那一行换成了短句。这一条对 192.168.213.11:8080 的 qwen3.8-27b 补录，模型仍然答 AFFIRM；换下来的旧条目已从录制文件删除。`recordings/glossary.json` 现在共 16 条，回放统计确认每一条都被用到。系统提示没有变，其余请求也没有变。
- **新增断言**：共 14 条，从 1126 条增加到 1140 条。
  - 「提问后再确认」场景加了 10 条。一是第一次念草稿全文念（改用逐字核对）。二是答疑返回后那次询问的话逐字等于短句、不含草稿，并且两次登记的上一问相同。三是第二次对话理解的对话历史里，告知那一行之后紧跟着短句。
  - 「提修改意见」场景加 1 条：改稿之后念的值变了，照常全文念第 2 稿，登记的念的值是第 2 稿。
  - 机器检查组加 3 条（从 40 条变成 43 条）。第一条直接调用询问：念的值变了就全文念，没变就说短句。第二条是中间隔着一次选择类登记时，草稿没变仍说短句。第三条是任务定义在「回复」槽位上写了「再问短句」时，再问说的就是那一句。
  - 含糊后选择、确认两个场景没有加断言，只是把登记上一问的预期值补上了「念的值」。
- **结果**：`python3 -m tod_kernel.verify` 回放全过，退出码 0，共 1140 条断言。kernel.py 没有改。
- **改动文件**：`tools.py`、`taskdef.py`、`task_defs/任务定义.schema.json`、`verify.py`、`task_defs/recordings/glossary.json`、`IMPLEMENTATION_NOTES.md`。

## G10 第五步收口前的提示词整治（2026-09-18 派发，四件事加对话历史改为话轮）

### G10.1 结论

- **结果**：四件事都做完了，对话历史的存储单位也改成了话轮。`python3 -m tod_kernel.verify` 回放全过，退出码 0，断言从 1140 条增加到 1152 条。kernel.py 没有改。
- **录制**：提示词变了，两份录制文件都从空文件开始，对 192.168.213.11:8080 的 qwen3.8-27b 整份重录。第一轮录完，标注集有一条变差（见 G10.5）。改了本步段里一句话之后又整份重录了一轮；现在仓里的录制是第二轮的：
  - `recordings/glossary.json` 共 16 条，其中文本形状 5 条，JSON 形状 11 条；
  - `recordings/对话理解标注集.json` 共 39 条。
  - 回放统计确认两份文件的每一条都被用到。
- **标注集两个准确率**：整治前是 1.000 和 1.000；第一轮录制降到 0.980 和 0.974；第二轮回到 1.000 和 1.000。明细见 G10.5。

### G10.2 提示词包的文件清单与格式

三个调模型的工具各一份提示词包，放在 `tod_kernel/prompts/`，文件名就是工具名：

| 文件 | 固定指令 | 本步的情形 | 片段 | 输出形状 |
|---|---|---|---|---|
| `prompts/对话理解.json` | 有 | 建议、求证、选择、请求、上一问为空 | 可写位置与选项的写法，共 8 个 | 没有：输出是 JSON，结构由代码现算 |
| `prompts/生成术语释义.json` | 有 | 首稿、改稿 | 进度·首稿、进度·改稿、参考材料缺失 | 有 |
| `prompts/答疑.json` | 有 | 解释 | 没有 | 有 |

- **格式**：一份包的顶层键是 工具、固定指令、本步、片段，文本形状的工具另有输出形状。
  - 本步与片段都是「键 → 模板」，占位符写成 `{名字}`。填空只用标准库的 `str.format_map`，模板里要出现字面的花括号时写成 `{{` 与 `}}`。
  - 用哪一个情形由代码选，模板里不写条件逻辑。
- **加载与核对**：新模块 `prompt_pack.py` 负责读包。每个工具的代码声明每个模板能提供哪些占位符（tools.py 里的 `DRAFT_PROVIDES`、`UNDERSTAND_PROVIDES`、`EXPLAIN_PROVIDES`），加载时逐个核对，下面几种情况都报提示词包错误（`PromptPackError`），并写明文件、位置与原因：
  - 情形或片段缺了，或者多了；
  - 模板用了代码提供不了的占位符；
  - 花括号写法不对；
  - 固定指令或输出形状里出现了占位符。
- **读包时机**：每个工具的包只读一次（`tools.prompt_pack_of`）。系统提示里工具目录的固定指令也从包里取。
- **代码侧的分工**：
  - context.py 的本步段不再自己拼句子，只把工具填好的一句话原样装进段。
  - 对话理解的情形选择与各占位符的取值写在 context.py 的 `understand_step_text` 与 `writable_places_text` 里，句子本身都在包里。
  - 输出 Schema 不进包。
- **另外改了一处进提示词的文字**：输出形状段的来源原来写「（同时作为接口参数 response_format 传给模型服务）」，里面有代码名。现在改成「（同一份结构也作为接口参数传给模型服务，输出必须符合它）」。

### G10.3 对话理解本步段的五种写法（模板原文，逐字）

占位符含义如下：
- {槽位}：上一问的槽位；
- {采纳到}：上一问登记的采纳位置；
- {值}：求证类上一问里待写的那个值；
- {选项}：选择类上一问的选项，按「序号 选项」用顿号连起来；
- {可写位置}：可写的槽位名，用顿号连起来，列表槽位另说明可以新增一项或改第几项。

某一项没有时填系统标记「（空）」。八个功能标识只出现在输出形状段（Schema 的枚举）和固定指令里。

**建议**

> 本步执行工具「对话理解」：系统刚才念了一份「{槽位}」，请使用者确认或提修改意见；使用者同意时，这份草稿会写进「{采纳到}」。现在请判断使用者的回答：可以是同意、否定、要换一份；也可以顺带给出内容（可写的位置：{可写位置}）、提问、推迟、无关的话；只有语气词或分不清意思时要澄清，不要当成同意。

**求证**

> 本步执行工具「对话理解」：系统刚才向使用者核对，使用者说的「{值}」是不是应该记成「{槽位}」。现在请判断使用者的回答：可以是同意（记得对）、否定（记得不对）；也可以顺带给出内容（可写的位置：{可写位置}）、提问、推迟、无关的话；只有语气词或分不清意思时要澄清，不要当成同意。

**选择**

> 本步执行工具「对话理解」：系统刚才请使用者从这几项里选一项：{选项}。现在请判断使用者的回答：可以是同意（选了其中一项，给出那一项的序号）、否定（哪一项都不要）；也可以顺带给出内容（可写的位置：{可写位置}）、提问、推迟、无关的话；只有语气词或分不清意思时要澄清，不要当成同意。

**请求**

> 本步执行工具「对话理解」：系统刚才请使用者提供「{槽位}」。现在请判断使用者的回答：可以是给出内容（可写的位置：{可写位置}）、否定（不提供）；也可以是提问、推迟、无关的话；只有语气词或分不清意思时要澄清，不要当成同意。

**上一问为空**

> 本步执行工具「对话理解」：系统这次没有向使用者提问，是使用者主动说了一句话。现在请判断这句话：可以是给出内容（可写的位置：{可写位置}）、提问、推迟、无关的话；只有语气词或分不清意思时要澄清。
...
| 位置 | `{槽位}` |
| 列表位置·没有项 | `{槽位}（可以新增一项）` |
| 列表位置·一项 | `{槽位}（可以新增一项，或改第 1 项）` |
| 列表位置·多项 | `{槽位}（可以新增一项，或改第 1 到第 {项数} 项）` |
| 位置之间 | `、` |
| 没有可写位置 | `没有` |
| 选项 | `{序号} {选项}` |
| 选项之间 | `、` |

以术语澄清为例，建议类那一问填好之后的本步段是：

> 本步执行工具「对话理解」：系统刚才念了一份「释义草稿」，请使用者确认或提修改意见；使用者同意时，这份草稿会写进「确认释义」。现在请判断使用者的回答：可以是同意、否定、要换一份；也可以顺带给出内容（可写的位置：术语、原文片段、修改意见、确认释义）、提问、推迟、无关的话；只有语气词或分不清意思时要澄清，不要当成同意。

与派发说明里的示例相比，改了下面几处：
1. 开头加了「本步执行工具「对话理解」：」。系统提示的角色段写明「【本步】段会指明这一步执行哪个工具」，所以每个工具的本步段都以这句开头。
2. 结尾由「分不清时要澄清。」改成「只有语气词或分不清意思时要澄清，不要当成同意。」，原因见 G10.5。上一问为空那种写法没有「同意」可选，结尾只到「要澄清。」为止。
3. 请求类的上一问本来就是要内容，所以把「给出内容」放在前面，不写成「顺带」。

### G10.4 固定指令与另外两个工具的本步段（定稿原文，逐字）

**对话理解的固定指令**（原来 Schema 里八段功能说明已并进来；原固定指令里的「候选功能集」「可写路径清单」改成了白话，另加一句路径的写法，因为 Schema 不再带说明）

> 你把使用者对系统上一问的回答变成一份对话行为列表。每项先定对话功能，再定语义内容。功能只能从【本步】段列出的几种里选，输出时写功能标识（下面每种功能括号前的英文词）；写值时槽位只能从【本步】段列出的可写位置里选，值按该槽位的类型给。路径的写法：写整个槽位是 []；列表槽位新增一项是 ["+"]，改第 n 项是 [n-1]，只改某一项里的一个字段时再接字段名。一句话可以有几项：对系统刚才那个问题的回应是一项，顺带说的别的事各是一项。改写了使用者原话的地方记进规范化修订。把握是你对这一项理解得对不对的估计，取 0 到 1。只输出 JSON。各功能的标识与用法如下。AFFIRM（同意）：接受上一问给的建议或值，内容为空，不要把建议原样再写一遍；上一问是选择时内容给所选那一项的选项序号（哪怕那一项的意思是修改或推迟）。使用者自己写出一段完整的新值来代替建议时也算同意，另加一项 INFORM 把新值写到采纳目标。DENY（否定）：不接受上一问。对建议提出改动要求的，另加一项 INFORM 把改动要求写进「修改意见」；说某个值不对并给了正确值的，另加一项 INFORM 写这个值；上一问是选择而使用者哪一项都不要时只用这一项。REQALTS（要换一份）：不说具体怎么改，只要求换一份或重写一份建议；内容为空，不另写修改意见。INFORM（给出内容）：写值，槽位与路径只能取可写的位置。REQUEST（提问）：只是问某个属性或词是什么意思，不写值；内容里的「问」给槽位名，或使用者问到的那个词的原话。DEFER（推迟）：先空着、以后再说，内容给要推迟的槽位。OTHER（无关的话）：闲聊、与任务无关，内容为空。CLARIFY（澄清）：只有语气词、或说法模棱两可，分不清是同意、否定还是别的意思时，只产出这一项，带一句问话和两三个选项，不要猜成同意。

**生成术语释义的固定指令**（没有改）

> 你是术语解释员。有原文片段时只根据原文片段解释这个术语；没有原文片段时按需求工程语境的通用含义解释。一到三句话，不引入片段之外的事实。若给了上一稿与修改意见，按修改意见改写上一稿，保留使用者没有要求改的内容。

**生成术语释义的本步段**：首稿、改稿两种

> 本步执行工具「生成术语释义」：为术语「{术语}」写一条释义草稿。

> 本步执行工具「生成术语释义」：当前数据里有上一稿与修改意见，请按修改意见改写上一稿，使用者没有要求改的内容保留。

片段：进度·首稿 `本次写第 {稿数} 稿。`；进度·改稿 `第 {上一稿} 稿被要求修改，本次写第 {稿数} 稿。`；参考材料缺失 `按通用含义解释`。输出形状：`只输出释义正文。`。这些短句的文字与整治前相同，只是从代码搬进了包里。

**答疑的固定指令**（没有改）

> 你是答疑员。使用者在任务进行中问了一个词或一句话是什么意思，问的内容在【本步】段里。结合当前数据里的槽位现值与对话历史，用一到两句白话说清它在这个任务里指什么：槽位现值或对话里有相关内容时照它解释，没有时按需求工程语境的通用含义解释。不反问，不评价也不改写草稿，不写入任何槽位。

**答疑的本步段**（原来是「使用者问：{问的内容}」，05 文档 4.12 节要同步改）

> 本步执行工具「答疑」：解释使用者问的这句话——{问的内容}

输出形状：`只输出一到两句白话解释正文。`。

### G10.5 标注集准确率：三次数字对比

| 时点 | 逐项功能准确率 | 逐句全对率 | 不一致的条目 |
|---|---|---|---|
| 整治前（G9.8 的录制） | 50/50 = 1.000 | 39/39 = 1.000 | 没有 |
| 第一轮重录：说明搬进固定指令、本步段改白话，结尾写「分不清时要澄清」 | 49/50 = 0.980 | 38/39 = 0.974 | 第 19 条「嗯。」：期望 CLARIFY，模型给了 AFFIRM |
| 第二轮重录：本步段结尾改成「只有语气词或分不清意思时要澄清，不要当成同意」 | 50/50 = 1.000 | 39/39 = 1.000 | 没有 |

第一轮时，「含糊后选择」场景也因为同一原因失败：「嗯。」被理解成同意，任务直接完成，没有进澄清。我的判断是这样的：整治前，「不要猜成同意」写在 Schema 里 CLARIFY 那一支的说明上，模型逐键生成时离它很近；现在这句话搬进了系统提示，离判断的位置远了。

第二轮只改了本步段结尾那一句，没有点名「嗯」这类具体用语，固定指令一字没动。改完后两份录制又都从空文件整份重录。

仍要提醒一点：这句话是看着第 19 条的错法补的，05 文档 4.5 节记的「贴合标注集」风险依然存在，而且多了一处。

### G10.6 Schema 精简

- `understand_schema` 去掉了全部说明文字（description）：原来每个功能那一支的说明，与「问」字段的说明，都删掉了。
- 功能相同形状合并成一支时，也不再拼接说明。
- 分支结构（anyOf）按派发说明没有改。
- tools.py 里的 `FUNCTION_NOTES`、`ASK_CONTENT_NOTE`、`UNDERSTAND_INSTRUCTION`、`understand_step_note` 都删了，`DRAFT_*`、`EXPLAIN_*` 那几句提示词常量也删了；工具表里的 `MODEL_INSTRUCTIONS` 换成了 `MODEL_TOOLS`（工具名 → 占位符声明与有没有输出形状）。

### G10.7 观测台：JSON 段折叠显示

- 段的正文如果是一份合法的 JSON（对象或数组），就折叠显示。折叠时显示一行摘要，例如「JSON（对象，4 个键，3400 字）· 点开看缩进后的全文」，点开后按两格缩进显示全文，框内可以滚动。
- 判断依据只看内容是不是 JSON，不看段名，所以以后别的段装 JSON 时也会这样显示。
- 协同记录卡里「提示词构成」的每一段，与「上下文变化」表的格子，用的是同一个显示函数。表头的说明改成了「本表文字截断显示、JSON 折叠显示」。
- 截图一张放在步骤目录：`截图_第五步观测台_输出形状渲染_1280.png`，1280 宽。取自「提问后再确认」那次运行，展开的是第一次对话理解（使用者问「产出」是什么意思）那张卡里的输出形状段。

### G10.8 对话历史改成话轮（并入本批）

- **存储单位**：context.py 删掉了 `Turn` 与 `turns_of` 的一问一答配对逻辑，改为话轮 `Utterance`。
  - 字段有说话方（系统或使用者）、原文、事件序号、所在阶段、所属工具调用编号。
  - 另外两个字段是实现三条规则要用的，是我加的：种类（提问、告知、回答），与写入槽位（提问的写入目标；回答跟着它的提问取这一项；告知没有）。
  - `utterances_of(events)` 按事件序号取出全部话轮：系统的提问、系统的告知、使用者的回答各一条，没有回答的提问也照记。
- **交互轮次**：不是存下来的对象，是划分函数 `exchanges_of(utterances)` 现算出来的 `Exchange`。从系统的一个提问到下一个提问之前的全部话轮为一段，中间的告知归前一段；第一个提问之前的话轮自成一段。
- **三条规则的算法**：
  - 按范围取，是先按话轮筛；筛剩的话轮再划成交互轮次。
  - 已在数据里的不重复装，按交互轮次判：这一段的回答原样留在它的写入槽位里时，整段不装。
  - 最近一段恒装。
  - 超预算从最早的交互轮次整段裁起。
  - 渲染仍是逐行「系统：…」「使用者：…」原文。
- **与改之前不同的地方**：
  1. 告知原来单算一轮，现在归进前一段，所以段头来源里「n 轮装入」的 n 与预算用字会变。「提问后再确认」场景第二次对话理解那条请求因此变了，它已包含在第二轮整份重录里。对话正文逐行的文字没有变。
  2. 没有回答的提问现在会进历史，段头多一项「没有回答的提问 n 个」（只在有时才写）。这次六个场景里没有出现这种情形。
  3. 如果一段的回答已在数据里、整段不装，挂在这一段后面的告知也跟着不装；原来告知从不因为这条规则被去掉。这次的场景里也没有出现这种情形。
- **新增机器检查两条**，放在「上下文包与对话历史的三条规则」组，这一组从 15 条变成 17 条：
  1. 告知与没有回答的提问都进话轮，逐条核对说话方、种类、原文、事件序号、阶段、工具调用编号。
  2. 交互轮次的划分：告知归前一段、开头的告知自成一段、渲染逐行原文。

### G10.9 断言与改动文件

- **断言**：从 1140 条增加到 1152 条，共 12 条。
  - 对话理解机器检查组加了 10 条（从 43 条变成 53 条）：
    - Schema 里没有说明文字，1 条；
    - 五种本步段各一例逐字核对，并且不含功能标识与代码名，5 条；
    - 可写位置的三种列表写法，1 条；
    - 占位符核对两例，模板多用一个提供不了的占位符报错、缺情形报错，2 条；
    - 三份包都能加载且固定指令进了工具目录，1 条。
  - 上下文包组加 2 条，见 G10.8。
  - 场景里原来核对「候选功能集」「可写路径清单」字样的断言，改成逐字核对白话本步段，条数不变。
- **改动文件**：
